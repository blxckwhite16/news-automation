...
**실패 경로 자체도 배포 전에 검증한다**: 위 원칙들은 "정상 경로에서 에러 출력이 어딘가에 연결되어 있는가"까지만 보장하고, "그 연결된 에러 처리 코드가 실제로 실패했을 때 올바르게 동작하는가"는 별도로 검증해야 한다 — 정상 경로가 아무리 오래 안정적으로 반복 실행돼도 에러 경로 자체가 실제로 실행된 적은 없을 수 있다. 실제로 `Save Raw News`의 에러 처리 노드가 회사 컨텍스트를 다른 노드의 pairedItem 참조로 복구하려다, 그 노드가 처음 실제로 실패한 날 예외를 재던져 전체 실행을 중단시킨 사고가 있었다(회귀가 아니라 최초 배포 이후 계속 존재했던 잠재 결함 — 자세한 조사·재현·수정 과정은 `docs/SESSION_SUMMARY.md` 2026-08-12 참고). 이 계기로 확립한 실패 경로 검증 원칙과 재사용 가능한 시나리오 매트릭스는 프로젝트 밖 공용 지식 문서 `knowledge/n8n/n8n-failure-path-testing.md`에 일반화되어 있다.
...
n8n은 로컬 Windows PC(Docker Desktop + WSL2) 위에서 상시 구동되는 것이 아니라, 매일 아침 트리거 시점에만 살아있으면 되는 구조다. 이 PC가 Modern Standby(S0) 전용 노트북이라 Windows Task Scheduler의 예약 기상(Wake Timer)이 구조적으로 신뢰할 수 없다는 문제가 실측으로 확인되었다(조사 과정과 근거는 SESSION_SUMMARY.md "Windows Host 전원 관리 및 S4 RTC Wake Bridge (2026-08-04)" 절 참고). 이 하드웨어가 지원하는 Hibernate(S4) + BIOS RTC Wake 조합은 실측으로 신뢰성이 확인되어 운영 방식으로 채택했고, 사용자도 배터리 절약 등의 이유로 취침 시 S4를 계속 선택해 쓰고 있다.

**[2026-08-14 정정] "S4가 무인 기상 성공의 필수조건"이라는 초기 결론은 철회한다.** 2026-08-12(Modern Standby/S0 상태로 밤을 보낸 것이 실측으로 확인됨)와 2026-08-14(사용자가 명시적으로 S4를 선택, Kernel-Power/Power-Troubleshooter 로그로 약 7시간22분의 단일 연속 절전 구간을 확인) 양쪽 모두 WakeBridge 완주와 n8n 08:30 Schedule Trigger 실행까지 실제로 성공했다 — 2026-08-14는 여기에 더해 `run_status=SUCCESS`와 Telegram 수신까지 포함한 최초의 무인 PROD E2E 성공 사례였다. 두 사례를 비교한 결과, 무인 기상의 신뢰성을 실질적으로 결정하는 것은 절전 방식(S0/S4) 자체가 아니라 Task Scheduler `StartBoundary`(현재 07:00)를 실제 n8n 트리거 시각(08:30)보다 충분히 앞당겨 Windows의 불확실한 기상/실행 지연(StartWhenAvailable catch-up)을 흡수하는 buffer였다는 것이 실측으로 확인되었다(경위와 로그 근거는 `docs/SESSION_SUMMARY.md` 2026-08-14 절 참고). **단, 이것이 "절전 방식은 완전히 무관하다"는 뜻은 아니다** — 지금까지 성공이 확인된 것은 S0/S4 각각 이 buffer 전략과 결합된 경우뿐이며, S4는 여전히 실측으로 신뢰성이 확인된 운영 방식(사용자의 실제 선택)으로 유지한다.

**[2026-08-16 추가] 운영 기본값을 S4로 정리한다.** 2026-08-15(일반 절전, 자동화 미실행·Telegram 미수신)와 2026-08-16(S4, 정상 실행·Telegram 정상 수신) 결과를 사용자가 보고했다(이번 라운드는 Claude가 로그 재조회로 독립 검증하지 않음, 상세는 `docs/SESSION_SUMMARY.md` "운영 기본값 — 일반 절전 재실패 확인 및 S4 채택 (2026-08-15~16)" 절 참고). 위 "필수조건 아님" 결론 자체는 유지한다(2026-08-12 S0 성공 사례가 여전히 존재하고, 8/15 실패 메커니즘은 로그로 확인되지 않았다) — 다만 반복 운영 안정성 면에서 S4 쪽 근거가 쌓여, 이 장비의 **운영 기본값은 최대절전(S4)** 으로 정리한다.
...
- **에러 처리 코드는 정상 경로와 별도로, 실제 실패를 유도해 배포 전에 검증한다 (8.2절)** — 회사 단위 오류 격리를 담당하는 노드 자신의 코드 결함이 실제로 실패가 발생한 날에야 드러나 전체 실행을 중단시킨 사고로 확인했다. 프로덕션 데이터·외부 서비스에 영향을 주지 않는 격리된 DEV 하네스로 실패를 의도적으로 재현해 검증하는 것을 표준 절차로 채택했다(방법론은 `knowledge/n8n/n8n-failure-path-testing.md`).
...
- **무인 기상 신뢰성의 핵심은 S4(Hibernate) 자체가 아니라 `StartBoundary` catch-up buffer다 [2026-08-14 정정]** — S0(2026-08-12)와 S4(2026-08-14) 양쪽 모두 StartBoundary를 n8n 트리거보다 충분히 앞당긴 조건에서 실제로 성공한 것을 확인해, "S4가 필수조건"이라는 이전 결론을 철회했다. S4는 여전히 채택된 운영 방식이지만, 그 근거를 "필수"가 아니라 "실측으로 신뢰성이 확인된 선택지"로 낮춰 잡는다 (8.4절). **[2026-08-16 추가]** 이후 S0 재실패(8/15)·S4 재성공(8/16) 사례가 추가되며, "필수조건 아님" 결론은 유지한 채 이 장비의 **운영 기본값은 S4로 정리한다** (8.4절).
